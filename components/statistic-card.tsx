...
export function StatisticCard({ icon: Icon, value, label, suffix = "" }: StatisticCardProps) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-0 right-0 w-24 h-24 bg-[#3b3f69]/[0.03] rounded-full -translate-y-1/2 translate-x-1/2 group-hover:scale-150 transition-transform duration-500" />

      <div className="relative w-16 h-16 rounded-2xl bg-[#3b3f69] flex items-center justify-center mb-5 group-hover:scale-110 transition-transform duration-300">
        <Icon className="w-8 h-8 text-[#ca1254]" />
      </div>
      <span className="text-4xl md:text-5xl font-bold text-[#3b3f69] mb-3">
        {displayValue}{suffix}
      </span>
